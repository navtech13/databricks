...
import { Image, RichText, Language, IconList } from "../../components"
...
const footer = {
  ...
  items: [
    <div className='h6 hidden flex-1 md:block' key={1}>
      <div className='first-column border-navy-04 w-16 border-l pl-2.5'>
        <Image {...careers.image} />
        <RichText>{careers.info}</RichText>
      </div>
    </div>,
    <div className='text-1.5 second-column flex-1 ' key={2}>
      <div className='flex max-w-[160px] flex-col gap-3 leading-5 md:leading-6 lg:gap-5'>
        <Image {...about.image} />
        <Language text={about.language.text} items={about.language.items} />
        <IconList variant='B' items={iconList} />
        <RichText>{about.info}</RichText>
      </div>
    </div>,
  ],
}
